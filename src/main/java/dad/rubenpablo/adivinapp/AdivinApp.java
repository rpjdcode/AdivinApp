...
public class AdivinApp extends Application {
	...
	private Alert numberLimitError() {
		Alert alerta = new Alert(AlertType.ERROR);
		alerta.setHeaderText("Error - Número superior al límite");
		alerta.setContentText("Has introducido un número superior a 100");
		return alerta;
	}
	...
	private void onAdivinarAction(ActionEvent e) {
		...
		try {
			...
			if (recogido > 100) {
				numberLimitError().showAndWait();
			} else if (recogido == aleatorio) {
				...
				intentos++;
				String msg = (aleatorio < recogido)
						? String.format("El número a adivinar es menor que %d.%nVuelve a intentarlo.", recogido)
						: String.format("El número a adivinar es mayor que %d.%nVuelve a intentarlo.", recogido);
